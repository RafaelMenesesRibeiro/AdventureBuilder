...
public class Processor {
	...
	// important to use a set to avoid double submission of the same renting when it
	...
	private void processInvoices() {
		...
		for (Renting renting : this.rentingToProcess) {
			if (!renting.isCancelled()) {
				...
				InvoiceData invoiceData = new InvoiceData(renting.getProviderNif(), renting.getNif(), renting.getVehicle().getPlate(),
